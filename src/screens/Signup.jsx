...
const Signup = () => {
  ...
  // Get the organizationId and userId from query parameters
  const organizationId = searchParams.get("organizationId");
  const userId = searchParams.get("userId");

  const isInvitedUser = location.pathname === "/invite-signin";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://api.accountsgoal.com/api/register",
        credentials,
        {
          headers: {
  ...
  const handleSubmitInvitedUser = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      const response = await axios.post(
        `https://api.accountsgoal.com/api/register-user-under-organization/${organizationId}/${userId}`,
        credentials,
        {
          headers: {
            "Content-Type": "application/json; charset= UTF-8",
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 h-screen ">
      <div className="h-full w-full overflow-y-auto -mt-12">
        ...
        <form className="px-8 md:px-10 lg:px-20 -mt-7 ">
          ...
          {error && (
            <p className="bg-red-500 text-center  text-white py-3 w-full mt-5 ">
              {error}
            </p>
          )}
          {sucess && (
            <p className="bg-green-500 text-center text-white py-3 w-full mt-5 ">
              {sucess}
            </p>
          )}
          ...
          <div className="flex flex-col gap-y-1 mt-4">
            <label className="font-inter" htmlFor="fullname">
              Full Name <span className="text-[#ed0202]">*</span>
            </label>
            <input
              type="text"
          ...
          <div className="flex flex-col gap-y-1 mt-4">
            <label className="font-inter" htmlFor="mobile">
              Phone Number{" "}
            </label>
            <input
              type="text"
          ...
          <div className=" relative flex flex-col gap-y-1 mt-4">
            <label className="font-inter" htmlFor="confirmpassword">
              Confirm Password <span className="text-[#ed0202]">*</span>
            </label>
            <input
              //type="password"
              type={showPassword ? "text" : "password"}
            ...
            <button
              type="button"
              onClick={togglePasswordVisibility}
              className="absolute right-3 top-1/2  transform -translate-y-1/2 bottom-96  "
            >
              {showPassword ? <FiEyeOff /> : <FiEye />}
            </button>
          </div>
          ...
          {organizationId && userId ? (
            <button
              onClick={handleSubmitInvitedUser}
              className={`${
                loading
