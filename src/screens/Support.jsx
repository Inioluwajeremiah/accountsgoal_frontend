...
const faqItems = [
  // ... (same FAQ items as before)
];

function Support() {
  const toggleQuestion = (event) => {
    const questionElement = event.target.closest(".faq-item");
    const answerElement = questionElement.querySelector(".faq-answer");
  ...
  return (
    <section id="faq" className="container py-10 md:px-10 mx-auto">
      ...
      {faqItems.map((item, index) => (
        <div
          key={index}
          className="faq-item mt-4 w-[90%] md:w-[70%] lg:w-[50%] mx-auto"
        >
          <div
          ...
          >
            ...
            <div className="w-6 h-6 text-black font-semibold text-sm md:text-lg">
              +
            </div>
          </div>
          <div className="faq-answer mt-2 text-sm md:text-base text-primary-accent-color overflow-hidden transition-all duration-300 max-h-0">
            <p>{item.answer}</p>
          </div>
        </div>
      ))}
  ...
  const [faqItems, setFaqItems] = useState([
    {
      ...
      "answer": "Open the Accountsgoal app or use the web interface. Log in and go to your dashboard. On the web, click the dashboard at the top right. On mobile, follow the prompt or use the plus icon on the map page.\n\nClick the plus icon to upload. Ensure your Excel file has headings like ACCOUNT_NAME, ADDRESS, CELEBRATIONS, EMAIL, MOBILE_CONTACT, and REVENUE. Click 'upload document' and select your file.\n\nStart the import process and wait for the data to be integrated. The imported data will appear in the map screen or relevant section."
    },
    ...
    <section id="faq" className="container py-10 md:px-10 mx-auto">
      ...
      {faqItems.map((item, index) => (
        ...
        >
          ...
          >
            <h3 className="txt-sm md:text-lg font-semibold">{item.question}</h3>
            <div
